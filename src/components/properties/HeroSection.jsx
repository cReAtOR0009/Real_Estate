...
const HeroSection = () => {
  const [searchParams, setsearchParams] = useState({Location:"", propertyType:"", calender:"", pricingRange:"", buildYear:""})

  const handleChange = (e, name)=> {
    setsearchParams(initialValue=> ({...initialValue, [name]:e.target.value}))
      ...
      <div className={` ${styles.subContainer} flex-col`}>
        ...
        <div className="flex flex-col sm:flex-wrap gap-[20px] justify-between mt-[20px] p-[20px] bg-Grey-10  rounded-[12px] md:flex-row lg:flex-nowrap md:mt-[-5px]">
          {searchinputs.map((searchinput, index) => (
            <SearchParameterInput key={index} {...searchinput} />
          ))}
        </div>
      </div>
    </>
